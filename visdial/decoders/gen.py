...
class GenerativeDecoder(nn.Module):
  def __init__(self, hparams, vocabulary):
    ...
    self.lstm_to_words = nn.Linear(self.hparams.lstm_hidden_size, len(vocabulary))
  ...
  def forward(self, encoder_output, batch):
    ...
    if self.training:
      ...
      init_hidden = init_hidden.repeat(
        self.hparams.lstm_num_layers, 1, 1
      )
      ...
      target_ans_len = torch.sum(target_ans_out > 0, dim=-1).view(-1)
      ans_scores = torch.sum(ans_word_scores, dim=-1) / target_ans_len
